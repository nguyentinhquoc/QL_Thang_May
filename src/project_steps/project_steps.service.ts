  ...
  async findProject (id: number) {
    const projectSteps = await this.projectStepRepository.find({
      ...
      relations: [
        ...
        'workflowStep.step.departmentsSteps',
        'workflowStep.step.departmentsSteps.department',
      ],
    })
    ...
    return projectSteps.map(projectStep => {
      ...
      const department = step ? step.departmentsSteps : null // Extract department
      return {
        ...projectStep,
        step,
        workflow,
        department, // Include department in the result
      }
    })
  }
